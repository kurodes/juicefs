...
func TestTierInfo_SetTierID_ValidValues(t *testing.T) {
	cases := []uint8{0, 1, 2, 3}

	for _, want := range cases {
		var ti TierInfo = 0x2A // 0010_1010，The lower 6 bits have a value, which is convenient for verification and ensures they are not destroyed.
		if err := ti.SetTierID(want); err != nil {
			t.Fatalf("SetTierID(%d) unexpected error: %v", want, err)
		}

		got := ti.GetTierID()
		if got != want {
			t.Fatalf("SetTierID(%d), GetTierID()=%d, want=%d", want, got, want)
		}
	}
}

func TestTierInfo_SetTierID_OutOfRange(t *testing.T) {
	cases := []uint8{4, 5, 255}

	for _, in := range cases {
		var ti TierInfo
		if err := ti.SetTierID(in); err == nil {
			t.Fatalf("SetTierID(%d) expected error, got nil", in)
		}
	}
}

func TestTierInfo_SetTierID_PreserveLower6Bits(t *testing.T) {
	// Initial value: 1010_1010
	// Lower 6 bits: 00101010 & 00111111 = 00101010 (0x2A)
	var ti TierInfo = 0xAA
	origLower6 := uint8(ti) & 0x3F

	if err := ti.SetTierID(1); err != nil {
		t.Fatalf("SetTierID(1) unexpected error: %v", err)
	}

	gotLower6 := uint8(ti) & 0x3F
	if gotLower6 != origLower6 {
		t.Fatalf("lower 6 bits changed: got=%08b want=%08b", gotLower6, origLower6)
	}
}

func TestTierInfo_GetTierID(t *testing.T) {
	cases := []struct {
		raw  uint8
		want uint8
	}{
		{raw: 0x00, want: 0}, // 00xxxxxx
		{raw: 0x40, want: 1}, // 01xxxxxx
		{raw: 0x80, want: 2}, // 10xxxxxx
		{raw: 0xC0, want: 3}, // 11xxxxxx
		{raw: 0xFF, want: 3}, // 11111111 -> 3
	}
	...
	for _, tc := range cases {
		...
		got := ti.GetTierID()
		if got != tc.want {
			t.Fatalf("GetTierID(%08b)=%d, want=%d", tc.raw, got, tc.want)
		}
	}
}
